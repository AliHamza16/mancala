...
function move(pocket) {
    ...
    const intv = setInterval(() => {
        if (
            board[(board.indexOf(pocket) + index) % board.length] ===
            `${a[turn]}-store`
        )
            bilmemNe = 1;
        const x =
            board[(board.indexOf(pocket) + index + bilmemNe) % board.length];
        lastStone = x;
        if (x === turn + '-store') {
            ...
            const count2 = Number(
                img2.src.split('Rocks/rock-')[1].split('.')[0],
            );
        ...
        if (index >= count) {
            ...
            function next() {
                ...
                if (!lastStone.includes('store')) {
                    ...
                    const ct = Number(
                        im.src.split('Rocks/rock-')[1].split('.')[0],
                    );
                    const c2 = document.querySelector(
                        `#${a[turn]}-${
                            7 - Number(lastStone.split('-')[1])
                        } > img`,
                    );
                    const t2 = Number(
                        c2.src.split('Rocks/rock-')[1].split('.')[0],
                    );
                    if (
                        ct === 1 &&
                        lastStone.startsWith(turn) &&
                        !lastStone.includes('store') &&
                        t2 !== 0
                    ) {
                        ...
                        const t1 = Number(
                            c1.src.split('Rocks/rock-')[1].split('.')[0],
                        );
                        const t2 = Number(
                            c2.src.split('Rocks/rock-')[1].split('.')[0],
                        );
...
function result() {
    available = false;
    const st1 = Number(
        document.querySelector(`#a-store > span`).innerText.trim(),
    );
    const st2 = Number(
        document.querySelector(`#b-store > span`).innerText.trim(),
    );
    const d = [st1, st2];
    const winner = players[d.indexOf(Math.max(...d))];
    if (st1 == st2) {
        document.querySelector('#modal-end').style.display = 'flex';
        document.querySelector('#modal-end > lottie-player').baseURI =
            'https://assets1.lottiefiles.com/packages/lf20_adhqfbqy.json';
        document.querySelector('#modal-end > lottie-player').play();
        document.querySelector('#modal-end > span').innerText = 'Tie';
    } else {
        document.querySelector('#modal-end').style.display = 'flex';
        document.querySelector('#modal-end > span').innerText = `${winner}`;
        document.querySelector('#modal-end > lottie-player').play();
    }
}
